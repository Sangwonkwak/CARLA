...
class BasicAgent(object):
    ...
    def __init__(self, vehicle, target_speed=20, estimated_transform=None, opt_dict={}, map_inst=None, grp_inst=None):
        """
        ...
        self._estimated_transform = estimated_transform
        if map_inst:
        ...
        self._local_planner = LocalPlanner(self._vehicle, opt_dict=opt_dict, map_inst=self._map, estimated_transform=self._estimated_transform)
        if grp_inst:
    ...
    def my_set_destination(self, end_location, start_location=None):
        """
        ...
        if not start_location:
            ...
            start_location = self._map.get_waypoint(self._estimated_transform.get_location())
            clean_queue = False
    ...
    def my_run_step(self):
        ...
        vehicle_speed = self._estimated_transform.get_speed()

        # Check for possible vehicle obstacles
        max_vehicle_distance = self._base_vehicle_threshold + self._speed_ratio * vehicle_speed
        # affected_by_vehicle, _, _ = self._vehicle_obstacle_detected(vehicle_list, max_vehicle_distance)
        affected_by_vehicle, _, _ = self.my_vehicle_obstacle_detected(vehicle_list, max_vehicle_distance)
        if affected_by_vehicle:
            hazard_detected = True

        # Check if the vehicle is affected by a red traffic light
        # max_tlight_distance = self._base_tlight_threshold + self._speed_ratio * vehicle_speed
        # affected_by_tlight, _ = self.my_affected_by_traffic_light(self._lights_list, max_tlight_distance)
        # if affected_by_tlight:
        #     hazard_detected = True

        # control = self._local_planner.run_step()
        control = self._local_planner.my_run_step()
        if hazard_detected:
    ...
    def my_affected_by_traffic_light(self, lights_list=None, max_distance=None):
        """
        ...
        ego_vehicle_location = self._estimated_transform.get_location()
    ...
    def my_vehicle_obstacle_detected(self, vehicle_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0):
        """
        ...
        ego_transform = self._estimated_transform
        # ego_location = ego_transform.location
        ego_location = ego_transform.get_location()
        ...
        for target_vehicle in vehicle_list:
            ...
            # Simplified approach, using only the plan waypoints (similar to TM)
                ...
                if is_within_distance(target_rear_transform, ego_front_transform, max_distance, [low_angle_th, up_angle_th]):
                    return (True, target_vehicle, compute_distance(target_transform.location, ego_transform.get_location()))
